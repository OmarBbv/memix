...
export default function AddProduct() {
  ...
  const onSubmit = (data: ProductFormValues) => {
    ...
    if (data.tags && data.tags.length > 0) {
      // Filter empty tags just in case
                    ...
                    render={({ field }) => (
                      <Input
                        ...
                        onChange={(e) => {
                          ...
                          const args = val.split(",").map(t => t.trim()); // don't filter empty yet to allow typing comma
                          field.onChange(args);
                        }}
                        onBlur={field.onBlur} // Important to trigger validation if any
                      />
                    )}
                  />
